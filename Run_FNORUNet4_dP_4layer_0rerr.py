...
parser.add_argument("-norm", "--normmethod", help="dP normalization method [minmax/zscore]", nargs='?', const="zscore", default="zscore", type=str)
...
t11 = default_timer()
print(f'Finished setup in {int(t11-t10)} sec')
#########################################################################################
# load and process data
#########################################################################################
# Load dP Data
hf_r = h5py.File(f'/home/groups/smbenson/shale_clean.hdf5', 'r')
...
if normmethod == "minmax":
    dP_min = np.min(data_dP_shale)
    dP_max = np.max(data_dP_shale)
    
    data_dP_shale = (data_dP_shale - dP_min)/(dP_max - dP_min)
...
t12 = default_timer()
print(f'Finished loading and processing data in {int(t12-t11)} sec')
#########################################################################################
# train model
#########################################################################################
...
writer = SummaryWriter(f'runs/dP3d_FNORUNet_{args.width}width_{args.mode1}m1_{args.mode2}m2_{args.mode3}m3_{train_nr}train_{args.epochs}eps_{args.errortype}err_{args.learnrate}lr_{args.zerror}zerr_{args.normmethod}norm_andrew_FNORUNet4_4layer_0rerr')
savefn = f'outputs/dP3d_FNORUNet_{args.width}width_{args.mode1}m1_{args.mode2}m2_{args.mode3}m3_{train_nr}train_{args.epochs}eps_{args.errortype}err_{args.learnrate}lr_{args.zerror}zerr_{args.normmethod}norm_andrew_FNORUNet4_4layer_0rerr.txt'
...
#             l2 += myloss2(dy_r_pred.reshape(dy_r_pred.shape[0], -1), dy_r.view(dy_r.shape[0], -1))
    ...
    if ep % 2 == 0 or ep == epochs-1:
        PATH = f'saved_models/dP3d_FNORUNet_{ep}ep_{args.width}width_{args.mode1}m1_{args.mode2}m2_{args.mode3}m3_{train_nr}train_{args.epochs}eps_{args.errortype}err_{args.learnrate}lr_{args.zerror}zerr_{args.normmethod}norm_andrew_FNORUNet4_4layer_0rerr'
